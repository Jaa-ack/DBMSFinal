    ...
    .then((data) => {
      ...
      if (data.length === 0) {
        ...
        data.forEach(function (meal) {
          var row = `<tr><td>${meal.food_name}</td><td>${meal.calories}</td><td><button class="bg-danger">Delete</button></td></tr>`;
    ...
    .then((data) => {
      ...
      if (data.length === 0) {
        ...
        data.forEach(function (exercise) {
          var row = `<tr><td>${exercise.type}</td><td>${exercise.calories}</td><td><button class="bg-danger">Delete</button></td></tr>`;
          tableBody.insertAdjacentHTML("beforeend", row);
        });
      }
    })
...
async function updatePassword() {
  ...
  if (password !== checkPassword) {
    document.getElementById("cpassword-feedback").innerText =
      "Passwords do not match";
    return;
  }
...
async function searchExercise() {
  ...
  try {
    const response = await fetch(
      `http://localhost:3000/exercises?text=${text}`
    );
    const data = await response.json();
